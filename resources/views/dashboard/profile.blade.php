...
<div class="max-w-4xl mx-auto space-y-6">
        ...
        <form method="POST" action="{{ route('dashboard.profile.update') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="flex items-start gap-6">
                <div class="flex-shrink-0">
                    <div class="relative">
                        @if(Auth::user()->photo)
                            <img id="preview-photo" src="{{ Storage::url(Auth::user()->photo) }}" alt="Profile Photo" class="w-24 h-24 rounded-full object-cover border-4 border-[#E5F0ED]">
                        @else
    ...
    <div class="bg-white rounded-2xl p-6 border-2 border-[#E5F0ED]">
        ...
        <form method="POST" action="{{ route('dashboard.profile.update-password') }}" class="space-y-6">
            @csrf
            @method('PUT')
