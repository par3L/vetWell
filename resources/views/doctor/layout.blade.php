...
<head>
    ...
    <title>@yield('title', 'Dashboard Dokter') - VetWell Clinic</title>
        ...
        <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-[#E5F0ED] transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0">
            <div class="flex flex-col h-full">
                ...
                <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                    <a href="{{ route('doctor.dashboard') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 {{ request()->routeIs('doctor.dashboard') ? 'bg-gradient-to-r from-[#2D7A6E] to-[#4A9FD8] text-white shadow-lg' : 'text-[#5A7A76] hover:bg-[#F0F8F6] hover:text-[#2D7A6E]' }}">
                ...
                <div class="p-4 border-t border-[#E5F0ED]">
                    <a href="{{ route('doctor.profile') }}" class="flex items-center gap-3 px-4 py-3 bg-[#F0F8F6] rounded-xl hover:bg-[#E5F0ED] transition-all duration-300 cursor-pointer">
                        @if(Auth::user()->doctor && Auth::user()->doctor->photo)
                            <img src="{{ Storage::url(Auth::user()->doctor->photo) }}" alt="{{ Auth::user()->name }}" class="w-10 h-10 rounded-full object-cover border-2 border-[#2D7A6E]">
                        @else
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-[#5A7A76]">Dokter</p>
                        </div>
                    </a>
            ...
            <header class="bg-white border-b border-[#E5F0ED] sticky top-0 z-30">
                <div class="flex items-center justify-between h-20 px-4 lg:px-8">
                    ...
                    <div class="flex-1 lg:flex-none">
                        <h1 class="text-xl lg:text-2xl font-bold text-[#1A3A35]">@yield('page-title', 'Dashboard Dokter')</h1>
                    </div>
                    ...
                    <div class="flex items-center gap-4">
                        <div class="hidden lg:flex items-center gap-2 px-4 py-2 bg-[#F0F8F6] rounded-full">
                            ...
                            <span class="text-sm font-medium text-[#1A3A35]">Dr. {{ Auth::user()->name }}</span>
                        </div>
                    </div>
                </div>
            </header>
